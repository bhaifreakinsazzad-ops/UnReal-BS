...
export async function GET() {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('id, requested_amount_bdt, note, status, created_at')
      .eq('user_id', userId)
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        user_id: userId,
        requested_amount_bdt: parsed.data.amountBdt,
    ...
    if (LOCATION_ID) {
      try {
        ...
        const response = await upsertContact(LOCATION_ID, {
          ...
          source: 'UnReal BS AI Subscriptions — Top-up Request',
        })
        const contactId = response.contact?.id
        if (contactId) {
          await addContactTags(contactId, LOCATION_ID, ['AI-TOPUP-REQUEST'])
        }
      } catch (ghlErr) {
        await logError('ai-subscriptions-topup-request-ghl-notify', ghlErr, { userId })
      }
    }
